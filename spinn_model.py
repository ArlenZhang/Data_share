...
class SPINN(nn.Module):
    ...
    def new_session(self, tree):
        ...
        tracking = self.tracker(stack, buffer, tracker_init_state)  # forward of Tracker
...
class SPINN_SR:
    def __init__(self):
        ...
        self.model = SPINN(SPINN_HIDDEN, word2ids, word_embed)
    ...
    def train(self, trees, trees_eval=None):
        ...
        for epoch in range(num_train_steps):
            ...
            for tree in trees:
                ...
                label = Var(torch.FloatTensor(gold_label))
                ...
                if iter_count % BATCH_SIZE == 0 and iter_count > 0:
                    ...
                    loss.backward()
                    optimizer.step()
                    optimizer.zero_grad()  # clear the grad history
                    ...
                    if batch_count % SKIP_STEP == 0:
                        ...
                        logger.log(logging.INFO, "[iter %-5d epoch %-3d  batch %-3d] train loss:%.5f eval loss:%.5f"
                                   % (iter_count, epoch, batch_count, loss.data[0] / BATCH_SIZE, eval_loss))
                        loss = 0.
                    yield batch_count
